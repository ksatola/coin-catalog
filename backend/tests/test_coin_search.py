...
def test_search_matches_category_ancestors_and_exact_scope(
    ...
    assert ids(client.get(f"/coins?search={root.name}&include_category_children=false")) == []
...
def test_category_filter_can_include_or_exclude_children(
    ...
    assert ids(client.get(f"/coins?category_id={root.id}&include_category_children=false")) == []
...
def test_sorting(
    ...
    assert ids(client.get(f"/coins?status=all&sort_by={sort_by}&sort_order={sort_order}")) == [
