  ...
  <ul class="coin-list">
    <li v-for="coin in coins" :key="coin.id" class="coin-row">
      ...
      <div class="coin-info">
        ...
        <div class="coin-line"><span v-for="(item, index) in formatIdentity(coin)" :key="`${coin.id}-identity-${index}`">{{ item }}</span></div>
        <div class="coin-line">
          <span v-for="(item, index) in formatDescription(coin)" :key="`${coin.id}-description-${index}`">{{ item }}</span>
          <svg v-if="coin.has_video" class="video-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-label="Video" role="img"><rect x="3" y="6" width="13" height="12" rx="2" /><path d="m16 10-5-3v10l5-3z" /></svg>
        </div>
      </div>
      <div class="actions">
        <button type="button" @click="emit('details', coin)">Szczegóły</button>
        <RouterLink class="button" :to="`/monety/${coin.id}/edytuj`">Edytuj</RouterLink>
        <button v-if="!coin.is_deleted" type="button" @click="emit('archive', coin)">Archiwizuj</button>
        <button v-else type="button" @click="emit('restore', coin)">Przywróć</button>
      </div>
    </li>
  </ul>
</template>
