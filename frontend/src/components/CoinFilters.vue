...
function filterQuery(): string {
  return buildCoinFilterQuery(props.filters)
}

defineExpose({ filterQuery, reload: loadFilters })
...
<template>
  <form class="coin-filters" @submit.prevent="submit">
    <label>
      Szukaj
      <input v-model="filters.search" type="search" placeholder="np. polska grosz" />
    </label>

    <div class="filter-grid">
      <label v-for="(items, name) in dictionaries" :key="name">
        {{ dictionaryLabels[name] }}
        <select v-model="filters.dictionarySelections[name as keyof typeof filters.dictionarySelections]" multiple size="4">
      ...
      <label>
        Kategorie
        <select v-model="filters.categoryIds" multiple size="4">
    ...
    <label>
      <input v-model="filters.includeCategoryChildren" type="checkbox" />
      Uwzględniaj podkategorie
    </label>

    <div class="range-filter">
      <label>Rok od <input v-model.number="filters.fromYear" type="number" /></label>
      <label>Rok do <input v-model.number="filters.toYear" type="number" /></label>
    </div>
    ...
    <div class="filter-row">
      ...
      <label>
        Wideo
        <select v-model="filters.hasVideo">
          <option value="">Dowolne</option>
    ...
    <div>
      ...
      <button type="button" @click="emit('submit')">Zastosuj</button>
    </div>
