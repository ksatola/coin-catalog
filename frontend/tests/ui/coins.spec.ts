...
async function mockCommonApi(page: Page) {
  ...
  await page.route(`**/api/coins/${coinId}/images/*/file`, async (route) => route.fulfill({ status: 200, contentType: 'image/jpeg', body: Buffer.from([0xff, 0xd8, 0xff, 0xd9]) }))
...
async function dropJpeg(page: Page, filename: string, zoneIndex = 0): Promise<void> {
...
test('podmiana awersu zostaje zapisana i widoczna po powrocie do listy', async ({ page }) => {
  ...
  await page.route(`**/api/coins/${coinId}`, async (route) => {
    if (route.request().method() === 'PUT') coinUpdates += 1
  ...
  expect(uploaded).toEqual([{ kind: 'avers', replace: true }])
})
...
test('podmiana rewersu zostaje wysłana jako zastąpienie', async ({ page }) => {
  ...
  expect(coinUpdates).toBe(1)
  expect(uploaded).toEqual([{ kind: 'rewers', replace: true }])
})
...
test('dodanie zdjęcia dodatkowego zostaje wysłane bez zastępowania', async ({ page }) => {
  ...
  await page.waitForURL(`/monety/${coinId}`)
  expect(coinUpdates).toBe(1)
  expect(uploaded).toEqual([{ kind: 'additional', replace: false }])
})
